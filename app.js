 ...
 function render(d){
  ...
  if(d.hero){ txt('h-line1',d.hero.line1); txt('h-line2',d.hero.line2); txt('h-lede',d.hero.lede); txt('h-cta',d.hero.ctaLabel);
   ...
   if(box && d.hero.images && d.hero.images.length){ box.innerHTML='';
    d.hero.images.forEach(function(src,i){ var s=document.createElement('div'); s.className='slide'+(i===0?' on':''); s.style.backgroundImage="url('"+src+"')"; s.setAttribute('data-fb',src.replace(/\.webp$/,'.jpg')); s.setAttribute('aria-hidden','true'); box.appendChild(s); }); }
  ...
  if(d.about){ txt('a-eyebrow',d.about.eyebrow); txt('a-line1',d.about.line1); txt('a-line2',d.about.line2); txt('a-p1',d.about.p1); txt('a-p2',d.about.p2); txt('a-link',d.about.linkLabel); var ai=id('a-img'); if(ai&&d.about.image){ var aipic=ai.parentElement; if(aipic&&aipic.tagName==='PICTURE'){ var aisrc=aipic.querySelector('source[type="image/webp"]'); if(aisrc) aisrc.srcset=d.about.image; } ai.src=d.about.image.replace(/\.webp$/,'.jpg'); } }
  if(d.collections){ var c=d.collections; txt('c-eyebrow',c.eyebrow); txt('c-heading',c.heading); txt('c-sub',c.sub); txt('c-cta',c.ctaLabel); txt('c-micro',c.micro);
   var g=id('coll-grid'); if(g && c.items && c.items.length){ g.innerHTML=''; c.items.forEach(function(it){ var card=document.createElement('div'); card.className='card'; var fig=document.createElement('figure'); var pic=document.createElement('picture'); var src=document.createElement('source'); src.type='image/webp'; src.srcset=it.image; pic.appendChild(src); var im=document.createElement('img'); im.src=it.image.replace(/\.webp$/,'.jpg'); im.alt=(it.title||'')+' pattern fabric'; pic.appendChild(im); fig.appendChild(pic); var cap=document.createElement('figcaption'); cap.textContent=it.title||''; card.appendChild(fig); card.appendChild(cap); g.appendChild(card); }); } }
